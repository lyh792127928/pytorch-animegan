...
class Transformer_openvino:
    def __init__(self, weight='checkpoint/animegan.onnx'):
        
        #weight 为对应的openvino模型（onnx或者ir）存放位置

        #方法一、二使用
        #self.core = Core()
        #self.model_inference = self.core.compile_model(model=weight)
        #self.weight = weight
        
        self.ie = Core()
        self.model = self.ie.read_model(model=weight)
    ...
    def transform(self, image):
        ...
        pred = denormalize_input(results.transpose(0, 2, 3, 1)[0],dtype=np.uint8)
